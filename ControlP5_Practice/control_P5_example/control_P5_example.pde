...
//text
  ...
  myChart = cp5.addChart("hello")
               .setPosition(x_pos-300, y_pos)
               .setSize(200, 200)
               .setRange(-20, 20)
               .setView(Chart.AREA) // use Chart.LINE, Chart.PIE, Chart.AREA, Chart.BAR_CENTERED
               ;
  myChart2 = cp5.addChart("world")
               .setPosition(x_pos, y_pos)
               .setSize(200, 200)
               .setRange(-20, 20)
               .setView(PIE) // use Chart.LINE, Chart.PIE, Chart.AREA, Chart.BAR_CENTERED
               ;
  ...
  myChart2.getColor().setBackground(color(255, 100));
  myChart2.addDataSet("world");
  myChart2.setColors("world", color(255,0,255),color(255,0,0));
  myChart2.setData("world", new float[4]);
  ...
  myChart2.setColors("earth", color(255), color(0, 255, 0));
  myChart2.updateData("earth", 1, 2, 10, 3);

  //println(cp5.ChartData);
...
void draw() {
  ...
  myChart.unshift("world", (sin(frameCount*0.05)*10));
  
  // push: add data from right to left (last in)
  myChart.push("earth", (sin(frameCount*0.08)*10));
  
  // unshift: add data from left to right (first in)
  myChart2.unshift("world", (sin(frameCount*0.05)*10));
  
  // push: add data from right to left (last in)
  myChart2.push("earth", (sin(frameCount*0.08)*10));
